...
ListRenderer.include({
                ...
                if (group.groupedBy.length) {
                    ...
                    var $records = _.map(group.data, function (record,index) {
                    	...
                    	if (_self.mode !== 'edit' || _self.hasSelectors){
                    		return self._renderRow(record).prepend($("<th class='o_list_row_count'>").html(index+1)); //.prepend($('<td>'));
                    	}
                    	else{
    ...
    _renderHeader: function (isGrouped) {
    	...
    	if (this.hasSelectors) {
    		$header.find("th.o_list_record_selector").before($('<th class="o_list_row_number_header o_list_row_count">').html('S.No'));
    		...
    		if (this.mode !== 'edit'){
    			$header.find("tr").prepend($("<th class='o_list_row_number_header o_list_row_count'>").html('S.No'));
    		}
    	}
